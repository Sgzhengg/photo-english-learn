...
export function VocabularyLibraryPage() {
  ...
  // Words and tags from API
  const [words, setWords] = useState<Word[]>([]);
  const [tags, setTags] = useState<Tag[]>([]);

  // Fetch words and tags from API on mount
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true);
        console.log('📚 [Vocabulary Library] Fetching words...');

        // Fetch words
        const wordsResult = await vocabularyApi.getWords();
        ...
        if (wordsResult.success && wordsResult.data) {
          ...
          console.error('❌ [Vocabulary Library] Failed to fetch words:', wordsResult.error);
        }

        // Fetch tags
        const tagsResult = await vocabularyApi.getTags();
